...
from booking.models import (WEEKDAYS, Booking, BookTime, Place, Building,
                            date_range, Validation, CATEGORY)
...
class SearchBookingForm(forms.Form):
    ...
    SEARCH_CHOICES = (
        ('opt_day_room', _(u'Grade Horária')),
        #('opt_booking_week', _(' Booking')),
        #('opt_building_day', _(' Departamento')),
        ('opt_room_period', _(' Recurso ')),
        #('opt_responsible', _(' Responsible')),
    )

    search_options = forms.ChoiceField(label=_('Opções de consulta'),
                                       choices=SEARCH_CHOICES,
                                       widget=forms.RadioSelect())

    building_name = forms.ModelChoiceField(
        queryset=Building.objects,
        label=_('Departamento:'), required=False,
        widget=forms.widgets.Select(
...
class BookingForm(forms.Form):
    ...
    hour = datetime.strptime("07:00", "%H:%M").time()
    hour2 = datetime.strptime("08:00", "%H:%M").time()
    hour3 = datetime.strptime("09:00", "%H:%M").time()
    hour4 = datetime.strptime("10:00", "%H:%M").time()
    hour5 = datetime.strptime("11:00", "%H:%M").time()
    hour6 = datetime.strptime("12:00", "%H:%M").time()
    hour7 = datetime.strptime("13:00", "%H:%M").time()
    hour8 = datetime.strptime("14:00", "%H:%M").time()
    hour9 = datetime.strptime("15:00", "%H:%M").time()
    hour10 = datetime.strptime("16:00", "%H:%M").time()
    hour11 = datetime.strptime("17:00", "%H:%M").time()
    hour12 = datetime.strptime("18:00", "%H:%M").time()
    hour13 = datetime.strptime("19:00", "%H:%M").time()
    hour14 = datetime.strptime("20:00", "%H:%M").time()
    hour15 = datetime.strptime("21:00", "%H:%M").time()
    hour16 = datetime.strptime("22:00", "%H:%M").time()
    hour17 = datetime.strptime("23:00", "%H:%M").time()
    HOURS = (('', '----'), (hour, '07:00'), (hour2, ('08:00')),
             (hour3, ('09:00')), (hour4, ('10:00')),
             (hour5, ('11:00')), (hour6, ('12:00')),
             (hour7, ('13:00')), (hour8, ('14:00')),
             (hour9, ('15:00')),(hour10, ('16:00')),
             (hour11, ('17:00')),(hour12, ('18:00')),
             (hour13, ('19:00')),(hour14, ('20:00')),
             (hour15, ('21:00')),(hour16, ('22:00')),
             (hour17, ('23:00')))
    ...
    name = forms.CharField(
        label=_('Nome da Reserva:'),
        widget=forms.TextInput(attrs={'placeholder': ''}))
    date_options = forms.ChoiceField(label=_('Deseja reservar para todo o\
                                                semestre?'),
...
class PlaceForm(forms.ModelForm):
    class Meta:
        model = Place
        fields = ('name', 'capacity', 'building', 'tipo',
               'possui_projetor', 'possui_quadro_branco', 'possui_quadro_negro', 'descricao')
